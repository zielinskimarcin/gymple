...
import { DbWorkout, getWorkoutById, insertWorkout, updateWorkoutDb, deleteWorkoutDb } from "../db/workouts";
import { setConfirmDone, setCancelDone } from "../storage"; // ⬅️ sygnał do Train
...
export const WorkoutDetailScreen = () => {
  ...
  const [loaded, setLoaded] = useState<DbWorkout | null>(null);
  ...
  async function confirmAndExit() {
    ...
    if (isPreview) {
      ...
      await setConfirmDone(savedId);           // ⬅️ sygnał dla Train, żeby wyczyścił stan
      // przejdź do root Tabs (zamiast goBack, żeby nie wracać do aktywnego treningu)
      // @ts-ignore
      nav.navigate("Tabs");
      return;
    }

    await updateWorkoutDb(
      w.id,
      { name: name || "Workout", payload },
      userId
    );
    nav.goBack();
  }
  ...
  async function handleDeleteWorkout() {
    if (!w || !userId) return;
    if (isPreview) {
      await setCancelDone();                   // ⬅️ sygnał anulowania
      // @ts-ignore
      nav.navigate("Tabs");
      return;
    }
          ...
          <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: spacing(2), paddingBottom: spacing(18) }}>
            ...
            <View style={s.headerCard}>
              <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                ...
                <Text style={{ color: colors.text, fontSize: 18, fontWeight: "800" }}>{isPreview ? "Workout done!" : "Workout"}</Text>
              </View>
                          ...
                          {!cardio ? (
                            ...
                            <>
                              <NumCounter
                                ...
                                onMinus={() => modSet(e.id, row.id, { distance: Math.max(0, Math.round(((row.distance ?? 0.5) - 0.1) * 10) / 10) })}
                                onPlus={() => modSet(e.id, row.id, { distance: Math.round(((row.distance ?? 0.5) + 0.1) * 10) / 10 })}
