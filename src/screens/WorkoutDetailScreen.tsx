...
export const WorkoutDetailScreen = () => {
  ...
  useEffect(() => {
    (async () => {
      if (isPreview) {
        ...
        const found = list.find((x) => x.id === workoutId) || null;
        setW(found);
        setName(found?.name ?? "");
      }
    })();
  }, [workoutId, isPreview, preview]);

  // Dodawanie nowego ćwiczenia z Search — TYLKO dla zapisanych (nie preview)
  useEffect(() => {
    (async () => {
      if (!focused || !w || isPreview) return;
      const picked = await popLastAddedExerciseTemp();
      if (picked) {
        setW((prev: any) => ({
          ...prev,
          exercises: [
            ...(prev?.exercises || []),
            { id: picked.id, name: picked.name, muscleGroup: picked.muscleGroup, sets: [] },
          ],
        }));
      }
    })();
  ...
  async function confirmAndExit() {
    ...
    if (isPreview) {
      ...
      const list = await loadWorkouts();
      await saveWorkouts([saved, ...list]);
      await setConfirmDone(savedId);
      nav.goBack();
      return;
    }
    await updateWorkout(w.id, () => ({ ...w, name }));
    nav.goBack();
  }
  ...
  function addSet(id: string) {
    ...
    setW({
      ...w,
      exercises: w.exercises.map((e: ExRow) => (e.id === id ? { ...e, sets: [...e.sets, def] } : e)),
    });
  }
  function modSet(exId: string, setId: string, delta: Partial<SetRow>) {
    ...
    setW({
      ...
      exercises: w.exercises.map((e: ExRow) =>
        e.id === exId ? { ...e, sets: e.sets.map((s) => (s.id === setId ? { ...s, ...delta } : s)) } : e
      ),
    });
  }
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
        <View style={{ flex: 1 }}>
          <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: spacing(2), paddingBottom: spacing(8) }}>
            <View style={s.top}>
            ...
            <View style={s.headerCard}>
              ...
              <Text style={{ color: colors.subtext, marginTop: 6 }}>
                {new Date(w.startedAt).toLocaleString()} • {Math.floor(w.durationSec / 60)}m {w.durationSec % 60}s
              </Text>
                ...
                <View key={e.id} style={s.card}>
                  ...
                  {!edit ? (
                        ...
                        <View key={row.id} style={s.setRow}>
                          ...
                          {!cardio ? (
                            <Text style={s.setText}>
                              {(row.weight ?? 0)} kg × {(row.reps ?? 0)}
                            </Text>
                          ) : (
                            <Text style={s.setText}>
                              {(row.distance ?? 0)} km · {(row.timeMin ?? 0)} min
                            </Text>
                          )}
                        </View>
                      ))
                    ...
                    <>
                          ...
                          {!cardio ? (
                            <>
                              <NumCounter
                                ...
                                value={row.weight ?? 20}
                                onMinus={() =>
                                  modSet(e.id, row.id, { weight: Math.max(0, (row.weight ?? 20) - 2.5) })
                                }
                            ...
                            <>
                              <NumCounter
                                ...
                                onMinus={() =>
                                  modSet(e.id, row.id, {
                                    distance: Math.max(0, round1((row.distance ?? 0.5) - 0.1)),
                                  })
                                }
                                onPlus={() =>
                                  modSet(e.id, row.id, { distance: round1((row.distance ?? 0.5) + 0.1) })
                                }
                      ...
                      {/* Delete całego ćwiczenia – tylko w trybie edycji */}
                      <TouchableOpacity onPress={() => removeExercise(e.id)} style={{ alignSelf: "center", marginTop: 6 }}>
                        <Text style={{ color: "#FF4D4D", textDecorationLine: "underline", fontWeight: "600" }}>
                          Delete
                        </Text>
                      </TouchableOpacity>
                    </>
            ...
            {/* Add exercise – tylko dla zapisanych (nie preview) */}
            {!isPreview && (
              <TouchableOpacity
              ...
              >
                ...
                <Text style={s.secondaryTxt}>Add exercise</Text>
              </TouchableOpacity>
            )}
          </ScrollView>

          {/* Dół: confirm + delete workout */}
...
const s = StyleSheet.create({
  ...
  // secondary button (Add exercise) – w ScrollView
  secondaryBtn: {
    flexDirection: "row",
    gap: 8,
    alignItems: "center",
  ...
  confirmBtn: { backgroundColor: colors.accent, borderRadius: 14, alignItems: "center", paddingVertical: spacing(2), marginTop: spacing(1) },
