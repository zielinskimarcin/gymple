...
export const WorkoutDetailScreen = () => {
  ...
  const { checkSaveLimit } = usePremium();
  ...
  useEffect(() => {
    (async () => {
      if (isPreview) {
        setW(preview);
        setName(preview?.name ?? t("workout.detail.defaultName"));
        return;
      }
      ...
      setName(row?.name ?? t("workout.detail.defaultName"));
    })();
  ...
  async function confirmAndExit() {
    ...
    if (isPreview) {
      ...
      const row: Omit<DbWorkout, "user_id"> = {
        id: savedId,
        name: name || t("workout.detail.defaultName"),
    ...
    await updateWorkoutDb(w.id, { name: name || t("workout.detail.defaultName"), payload }, userId);
    nav.goBack();
  }
  ...
  const startedText = new Date(w.startedAt).toLocaleString();
  const durMin = Math.floor((w.durationSec || 0) / 60);
  const durSec = (w.durationSec || 0) % 60;
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
        <View style={{ flex: 1 }}>
          <ScrollView
            style={{ flex: 1 }}
            contentContainerStyle={{ padding: spacing(2), paddingBottom: spacing(18) }}
          >
            <View style={s.top}>
              <TouchableOpacity onPress={() => nav.goBack()} style={s.iconBtn}>
                <Ionicons name="chevron-back" size={20} color={colors.text} />
              </TouchableOpacity>
              <Text style={s.topTitle}>{t("workout.detail.title")}</Text>
              <View style={{ width: 36 }} />
            </View>
            ...
            <View style={s.headerCard}>
              <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                ...
                <Text style={{ color: colors.text, fontSize: 18, fontWeight: "800" }}>
                  {isPreview ? t("workout.detail.doneTitle") : t("workout.detail.workoutTitle")}
                </Text>
              </View>

              <Text style={{ color: colors.subtext, marginTop: 6 }}>
                {startedText} • {durMin}
                {t("workout.detail.minutesShort")} {durSec}
                {t("workout.detail.secondsShort")}
              </Text>

              <Text style={s.label}>{t("workout.detail.nameLabel")}</Text>
              <TextInput
                value={name}
                onChangeText={setName}
                placeholder={t("workout.detail.namePlaceholder")}
                ...
                <View key={e.id} style={s.card}>
                  <View style={s.cardHeader}>
                    ...
                    <TouchableOpacity onPress={() => toggleEditExercise(e.id)}>
                      <Ionicons
                        name={edit ? "checkmark-done-outline" : "create-outline"}
                        size={18}
                  ...
                  {!edit ? (
                    e.sets?.length ? (
                      e.sets.map((row: SetRow, idx: number) => (
                        <View key={row.id} style={s.setRow}>
                          <Text style={s.setIndex}>{idx + 1}</Text>
                          {!cardio ? (
                            <Text style={s.setText}>
                              {(row.weight ?? 0)} {unit} × {(row.reps ?? 0)}
                            </Text>
                          ) : (
                            <Text style={s.setText}>
                              {(row.distance ?? 0)} {t("workout.detail.km")} · {(row.timeMin ?? 0)} {t("workout.detail.min")}
                            </Text>
                          )}
                        </View>
                      ))
                    ) : (
                      <Text style={{ color: colors.subtext }}>{t("workout.detail.noSets")}</Text>
                    )
                  ) : (
                    <>
                      {e.sets?.map((row: SetRow, idx: number) => (
                          ...
                          {!cardio ? (
                            <>
                              <NumCounter
                                ...
                                onMinus={() =>
                                  modSet(e.id, row.id, {
                                    weight: Math.max(0, (row.weight ?? 20) - weightStep),
                                  })
                                }
                                onPlus={() =>
                                  modSet(e.id, row.id, {
                                    weight: (row.weight ?? 20) + weightStep,
                                  })
                                }
                                onType={(v) => modSet(e.id, row.id, { weight: v })}
                              />
                              <NumCounter
                                label={t("workout.detail.reps")}
                                mode="int"
                                maxDigits={4}
                                value={row.reps ?? 8}
                                onMinus={() =>
                                  modSet(e.id, row.id, { reps: Math.max(0, (row.reps ?? 8) - 1) })
                                }
                            ...
                            <>
                              <NumCounter
                                label={t("workout.detail.km")}
                                value={row.distance ?? 0.5}
                                onMinus={() =>
                                  modSet(e.id, row.id, {
                                    distance: Math.max(
                                      0,
                                      Math.round(((row.distance ?? 0.5) - 0.1) * 10) / 10
                                    ),
                                  })
                                }
                                onPlus={() =>
                              ...
                              />
                              <NumCounter
                                label={t("workout.detail.min")}
                                value={row.timeMin ?? 5}
                                onMinus={() =>
                                  modSet(e.id, row.id, { timeMin: Math.max(0, (row.timeMin ?? 5) - 1) })
                                }
                      ...
                      <TouchableOpacity style={s.addSetBtn} onPress={() => addSet(e.id)}>
                        ...
                        <Text style={s.addSetTxt}>{t("workout.detail.addSet")}</Text>
                      </TouchableOpacity>
                      ...
                      >
                        <Text style={{ color: "#ff6b6b", fontWeight: "600" }}>
                          {t("workout.detail.deleteExercise")}
                        </Text>
                      </TouchableOpacity>
                    </>
                  )}
            ...
            <View style={s.addExerciseBar}>
              <TouchableOpacity
                activeOpacity={0.85}
                onPress={() => nav.navigate("SearchExercise")}
                style={s.addExerciseTouch}
              >
                <Ionicons name="search-outline" size={16} color="#fff" />
                <Text style={s.addExerciseTxt}>{t("workout.detail.addExercise")}</Text>
              </TouchableOpacity>
            </View>
          </ScrollView>
          ...
          <View style={s.bottomDock}>
            <TouchableOpacity style={s.confirmBtn} onPress={confirmAndExit}>
              <Text style={s.confirmTxt}>{t("workout.detail.confirm")}</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={handleDeleteWorkout} style={{ alignSelf: "center", marginTop: 8 }}>
              <Text style={{ color: colors.subtext, textDecorationLine: "underline" }}>
...
function NumCounter({
  ...
  return (
    <View style={s.counter}>
      ...
      <Text style={s.counterLabel} numberOfLines={1}>
        {label}
      </Text>
    </View>
  );
